...
export const createConfigurationTransaction = async (
  ...
  // Create a configuration UTxO that will serve as the protocol's bootstrap/genesis transaction
  const configAddress = await blaze.wallet.getChangeAddress();

  console.log(`Configuration details:`);
  console.log(
    `  Proposal Lifetime: ${PROPOSAL_LIFETIME} milliseconds (${PROPOSAL_LIFETIME / (1000n * 60n * 60n * 24n)} days)`,
  );
  console.log(`  Configuration Address: ${configAddress.toBech32()}`);

  // Create a simple UTxO that will serve as our protocol boot UTxO
  // This UTxO will later be consumed when minting the state NFT
  ...
  console.log(
    `This transaction ID should be used as PROTOCOL_BOOT_TRANSACTION_ID`,
  );
...
const main = async () => {
  ...
  try {
    ...
    console.log("3. Mint the state NFT with: bun run mint-state-nft");
    console.log("4. Try the deposit and withdrawal transactions");
