...
export class OgmiosTransactionSubmitter {
  ...
  async submitTransaction(builtTx: any, witnessSet: any): Promise<OgmiosSubmissionResult> {
      ...
      ws.on('open', async () => {
        try {
          ...
          try {
            ...
            console.log('Constructing complete transaction CBOR...');
            console.log(`Original tx CBOR: ${originalTxCbor.length} chars`);
            console.log(`Witness CBOR: ${witnessCbor.length} chars`);
            ...
            console.log(`Constructed tx has aux data: ${hasAuxData}`);
            console.log(`Body CBOR: ${txBodyCbor.length} chars`);
            console.log(`Final transaction CBOR: ${fullTxCbor.length} chars`);
            
          } catch (originalError) {
            console.log('Built transaction CBOR failed, constructing manually...');
          ...
          console.log(`Final transaction CBOR: ${fullTxCbor.length} chars`);
          console.log(`CBOR starts with: ${fullTxCbor.substring(0, 20)}...`);
          ...
          console.log('Submitting via Ogmios WebSocket...');
      ...
      ws.on('message', (data) => {
        if (!resolved) {
          ...
          try {
            ...
            if (response.error) {
              console.log(`Ogmios error: ${JSON.stringify(response.error)}`);
              ws.close();
              resolve({ 
                success: false, 
              ...
              console.log(`Ogmios success: ${txId || 'Transaction submitted'}`);
              ws.close();
              resolve({ 
                success: true, 
                txId: txId || 'unknown' 
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('message', (data) => {
        ...
        try {
          ...
          if (response.error) {
            reject(new Error(`Ogmios evaluation error: ${response.error.message}`));
          } else {
